---
name: test-coverage
description: "Audit a repository's test suite and generate real coverage analysis: detect the language(s) and testing framework, run tests with coverage instrumentation, produce line/function coverage reports (terminal + HTML + machine-readable), and identify untested critical paths with a prioritized gap-closing plan saved into the repo. Includes a dedicated, empirically-verified methodology for Bash/shell projects (bats-core), where naive kcov+bats coverage is known-unreliable, plus a shell-specific test-quality checklist covering set -e interaction bugs, unmocked external commands, and silent dispatch no-ops."
---
